$('document').ready(function() {
    ...
    .on('click', function(e) {
        ...
        // create tornadoMarker
        var tornadoMarker = L.AwesomeMarkers.icon({
            icon: 'coffee',
            markerColor: 'blue'
        });
        var circle = L.circle([35.2226, -97.4395], {
            color: 'red',
            fillColor: '#f03',
            fillOpacity: 0.5,
            radius: 500
        ...
        // add tornadoMarker to map
        // L.marker([lat, lng], { icon: tornadoMarker }).addTo(map);

        // append svg to tornadoMarker
        // $('.awesome-marker-icon-blue').append(tornadoSVG);
    })
    ...
    L.easyButton('fa-compass',
        ...
        function() {
            ...
            map.once('click', function(e) {
                ...
                var hailMarker = L.AwesomeMarkers.icon({
                    icon: 'coffee',
                    markerColor: 'blue'
                });

                // add hailMarker to map
                // L.marker([lat, lng], { icon: hailMarker }).addTo(map);
                // $('.awesome-marker-icon-blue').append(hailSVG);
            })
        },
        'Interact with the map'
    ).addTo(map);
    ...
    var tornadoControl = L.easyButton({
        states: [{
            ...
            onClick: function(btn, map, e) {
                $('#tornado-event').prop('checked', true);
                $('.tornado-control').prop('checked', true);
